...
# Separate day 0 and day 1 data for training
day_0_train = train_df[train_df['planned_day_relative_to_boost'] == 0].set_index('subject_id')
day_14_train = train_df[train_df['planned_day_relative_to_boost'] == 1].set_index('subject_id')

# Separate day 0 and day 1 data for testing
day_0_test = test_df[test_df['planned_day_relative_to_boost'] == 0].set_index('subject_id')
day_14_test = test_df[test_df['planned_day_relative_to_boost'] == 1].set_index('subject_id')
...
y_train = day_14_train['Monocytes']
...
# Use the best model for the unseen dataset
new_df = pd.read_csv(testing_input_path)  # Using the second argument for the testing dataset
...
results_to_merge = results_to_merge.rename(columns={
    'Predicted_Monocytes_Rank': '2.1) Monocytes-D1-Rank',
    'Fold_Change_Rank': '2.2) Monocytes-D1-FC-Rank'
})

updated_df = existing_df.merge(results_to_merge, on='SubjectID', how='left')

if '2.1) Monocytes-D1-Rank_x' in updated_df.columns:
    updated_df['2.1) Monocytes-D1-Rank'] = updated_df['2.1) Monocytes-D1-Rank_y']
    updated_df.drop(columns=['2.1) Monocytes-D1-Rank_x', '2.1) Monocytes-D1-Rank_y'], inplace=True)

if '2.2) Monocytes-D1-FC-Rank_x' in updated_df.columns:
    updated_df['2.2) Monocytes-D1-FC-Rank'] = updated_df['2.2) Monocytes-D1-FC-Rank_y']
