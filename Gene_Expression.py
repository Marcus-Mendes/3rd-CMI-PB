...
# 2) Testing dataset path (unseen dataset)
...
# Separate day 0 and day 3 data for training
day_0_train = train_df[train_df['planned_day_relative_to_boost'] == 0].set_index('subject_id')
day_14_train = train_df[train_df['planned_day_relative_to_boost'] == 3].set_index('subject_id')

# Separate day 0 and day 3 data for testing
day_0_test = test_df[test_df['planned_day_relative_to_boost'] == 0].set_index('subject_id')
day_14_test = test_df[test_df['planned_day_relative_to_boost'] == 3].set_index('subject_id')
...
y_train = day_14_train['ENSG00000277632.1']
...
y_test = day_14_test['ENSG00000277632.1']
...
# Since the data is already normalized, we do not need to scale it
...
new_df = pd.read_csv(testing_input_path)  # Using command-line argument for the testing dataset path
...
# Prepare features from day 0 data
...
monocytes_day_0 = day_0_new.loc[new_indices]['ENSG00000277632.1']
...
results_to_merge = results_to_merge.rename(columns={
    'Predicted_Monocytes_Rank': '3.1) CCL3-D3-Rank',
    'Fold_Change_Rank': '3.2) CCL3-D3-FC-Rank'
})

updated_df = existing_df.merge(results_to_merge, on='SubjectID', how='left')

if '3.1) CCL3-D3-Rank_x' in updated_df.columns:
    updated_df['3.1) CCL3-D3-Rank'] = updated_df['3.1) CCL3-D3-Rank_y']
    updated_df.drop(columns=['3.1) CCL3-D3-Rank_x', '3.1) CCL3-D3-Rank_y'], inplace=True)

if '3.2) CCL3-D3-FC-Rank_x' in updated_df.columns:
    updated_df['3.2) CCL3-D3-FC-Rank'] = updated_df['3.2) CCL3-D3-FC-Rank_y']
    updated_df.drop(columns=['3.2) CCL3-D3-FC-Rank_x', '3.2) CCL3-D3-FC-Rank_y'], inplace=True)
